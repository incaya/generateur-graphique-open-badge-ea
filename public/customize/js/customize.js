class CustomBadge {
    ...
    for (var lKey in this.layers) {
      if (lKey !== "id" && this.layers.hasOwnProperty(lKey)) {
        ...
        if (this.layers[lKey].position) {
          let xTranslate = this.layers[lKey].translate ? this.layers[lKey].translate.x : 0;
          let yTranslate = this.layers[lKey].translate ? this.layers[lKey].translate.y : 0;
          console.log("SETPOS: "+lKey, l.cx(), l.cy(), xTranslate, yTranslate);
  ...
  setText(layerId, captions, params) {
    ...
    console.log("POS: ", layerPosition.x, layerPosition.y);
  ...
  setColor(layerId, color) {
    console.log("Set color to : ", layerId, this.layers[layerId]);
